...
class PolicyNetwork(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_dim=256):
        ...
        # Add log_alpha as a trainable parameter
        self.log_alpha = nn.Parameter(torch.tensor(0.0))  # Initialize log_alpha

    def forward(self, state):
        x = F.relu(self.fc1(state))
        x = F.relu(self.fc2(x))
        mean = self.mean(x)
        log_std = torch.tanh(self.log_std(x))  # 標準偏差を安定化
...
class QFunction(nn.Module):
    ...
    def forward(self, state, action):
        x = torch.cat([state, action], dim=-1)  # 状態と行動を結合
        x = F.relu(self.fc1(x))
        x = F.relu(self.fc2(x))
        q_value = self.fc3(x)
        return q_value
